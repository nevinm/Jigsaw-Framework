...
$.fn.splitter = function(args){
	...
	return this.each(function() {
		...
		function endSplitMouse(evt) {
			...
			$(document).trigger('manually-resize' + opts.eventNamespace);
		}
		...
		function resplit(pos) {
	        ...
	        // }
	        //if (bar.hasClass('docked') || (!initialized && (opts.sizeBottom == 0 || opts.sizeRight == 0))) {
	        ...
	        if (bar.hasClass('docked')) {
	            ...
	            var dockSize = opts.dockSize || 0;

	            bar.removeClass(opts.barDockedClass);
	            bar._DA = bar[0][opts.pxSplit];
	            bar._pos = null;

	            pos = (opts.dockPane == A) ? dockSize : splitter._DA - dockSize;
	        }
            else {
        ...
        var bar = $('<div></div>')
            ...
            .css({position: "absolute", "user-select": "none", "-webkit-user-select": "none",
                "-khtml-user-select": "none", "-moz-user-select": "none", "z-index": "1"})
		...
		$(document).bind('animating', function (e, duration) {
		    animating = true;
		    setTimeout(function () {
		        animating = false;
		        if ( opts && opts.type == 'v' && A) {
		            //console.log(A.width());
		            resize(A.width());
		        }
    	    }, duration);
		...
		splitter
	        ...
	        .bind("mandatory-resize" + opts.eventNamespace, function () { mandatoryResize(); })
